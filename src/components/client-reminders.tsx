...
import { Bell, Plus, Check, Trash2, Clock, X, ChevronDown, ChevronUp } from "lucide-react"
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground text-center py-4">Aucun rappel en attente</p>
        ) : (
          <div className="space-y-2">
            {pending.map(r => {
              const lbl = echeanceLabel(r.echeance)
              return (
                <div key={r.id} className="flex items-start gap-2 p-2 rounded-lg border bg-card">
                  <button
                    onClick={() => handleDone(r.id, r.fait)}
                    className="mt-0.5 w-5 h-5 flex-shrink-0 rounded-full border-2 border-muted-foreground/40 hover:border-primary hover:bg-primary/10 transition-colors"
                  />
              ...
              <div className="space-y-1 mt-2">
                {done.map(r => (
                  <div key={r.id} className="flex items-center gap-2 p-2 rounded-lg opacity-50">
                    ...
                    <span className="text-sm line-through flex-1">{r.titre}</span>
                    <button onClick={() => handleDelete(r.id)} className="text-muted-foreground hover:text-red-500">
